"""Train RowSongUNet on all rows of a single image with TensorBoard visualization.

Uses SingleImageRowDataset to iterate over every row of the source image.
After training, logs a figure with 25 random ground-truth rows vs 25
generated rows (line plots, x = pixel index, y in [-1, 1]).

Usage:
    torchrun --standalone --nproc_per_node=1 train_all_rows_pipeline.py \
        --image datasets/one-image-10k-src/000000.png --outdir training-runs
"""
...
from training.dataset import SingleImageRowDataset
...
def make_grid_figure(gt_rows, gen_rows, row_indices):
    ...
    Each subplot: x = pixel index, y = value in [-1, 1].
    gt_rows, gen_rows: numpy arrays of shape [25, C, 1, W].
    row_indices: list/array of 25 row indices for subplot titles.
    """
    n = len(row_indices)
    cols = 5
    rows = (n + cols - 1) // cols
...
def main(**kwargs):
    ...
    # --- Dataset + DataLoader ---------------------------------------------
    dataset = SingleImageRowDataset(path=opts.image, cache=True)
    C = dataset.num_channels
    W = dataset.row_resolution
    num_rows = len(dataset)
    print(f'Image: {opts.image}  ({C}ch, {num_rows} rows, width {W})')
    ...
    desc = f'{dataset_name}-allrows-fp32'
    prev_run_dirs = []
    ...
    # --- Training loop ----------------------------------------------------
    print(f'Training for {opts.steps} steps on all {num_rows} rows ...')
    ...
    gt_rows = np.stack(gt_rows)  # [25, C, 1, W]

    with torch.no_grad():
        latents = torch.randn([len(viz_indices), C, 1, W], device=device)
        gen_rows = edm_sampler(net, latents, num_steps=100).cpu().numpy()  # [25, C, 1, W]
